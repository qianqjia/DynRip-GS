...
from gaussian_renderer.render_time import render, prefilter_voxel
import torchvision
from utils.general_utils import safe_state
from argparse import ArgumentParser
from arguments import ModelParams, PipelineParams, get_combined_args
from scene.gaussian_model_uw import GaussianModel
...
def record_set(model_path, name, iteration, views, gaussians, pipeline, background, out_path='out', is_train=True):
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    fps = 30
    frame_size = (views[0].image_width, views[0].image_height)
    out = cv2.VideoWriter(out_path + '_{}_image.mp4'.format('train' if is_train else 'test'), fourcc, fps, frame_size)
    # out_normal = cv2.VideoWriter(out_path + '_{}_normal.mp4'.format('train' if is_train else 'test'), fourcc, fps, frame_size)
    ...
    for view_pre, view_cur in zip(views[:-1], views[1:]):
        ...
        for t in range(20):
            ...
            # rendering = render_pkg["surf_depth"]
            # rendering_normal = render_pkg["surf_normal"]
            # color = cv2.applyColorMap((tensor2image(rendering/30) * 255).astype(np.uint8), cv2.COLORMAP_JET)
            color = cv2.cvtColor((np.clip(tensor2image(rendering), 0, 1) * 255).astype(np.uint8), cv2.COLOR_BGR2RGB)
            # normal = cv2.cvtColor((np.clip(tensor2image(rendering_normal * 2 - 1), 0, 1) * 255).astype(np.uint8), cv2.COLOR_BGR2RGB)
            out.write(color)
            # out_normal.write(normal)
...
def render_record(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, out_path = 'out'):
    ...
    if not skip_train:
        record_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, out_path, is_train=True)
    
    if not skip_test:
        record_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, out_path, is_train=False)
