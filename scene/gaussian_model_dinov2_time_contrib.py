...
def time_encoding(t: torch.Tensor, num_freqs: int = 6) -> torch.Tensor:
    """
    输入: t [B, 1]
    输出: [B, num_freqs*2]
    """
    freq_bands = (
        2 ** torch.arange(num_freqs).float().cuda()
    )  # [0, 1, 2, ..., num_freqs-1]
    x = 2 * np.pi * t  # convert to radians if desired
    out = [torch.sin(x * f) for f in freq_bands] + [
        torch.cos(x * f) for f in freq_bands
    ]
...
class GaussianModel:
    ...
    def __init__(
        ...
        self.offset_abs_gradient_accum = torch.empty(0)
        ...
        self.mlp_color = nn.Sequential(
            ...
            nn.Sigmoid(),  # 输出RGB颜色，范围[0,1]
        ).cuda()

        self.time_dim = 10
        # 定义时间相关的mlp，输入是特征、位置、时间
        self.mlp_color_time = nn.Sequential(
            nn.Linear(
                feat_dim
                + 3
                + self.color_dist_dim
            ...
            nn.ReLU(True),
            nn.Linear(feat_dim, 3 * self.n_offsets),
            nn.Tanh(),  # 输出颜色增量，范围[-1,1]
        ).cuda()

        self.uncertainty_model = UncertaintyModel(uncertainty_config).cuda()
    ...
    def get_parameters(self):
        ...
        params += list(self.mlp_cov.parameters())
        params += list(self.mlp_color.parameters())
        params += list(self.mlp_color_time.parameters())
        if self.use_feat_bank:
            params += list(self.mlp_feature_bank.parameters())
        if self.appearance_dim > 0 and self.embedding_appearance is not None:
            params += list(self.embedding_appearance.parameters())
        return params
    ...
    @property
    def get_color_time_mlp(self):
        return self.mlp_color_time
    ...
    def training_setup(self, training_args):
        ...
        self.offset_gradient_accum = torch.zeros(
            (self.get_anchor.shape[0] * self.n_offsets, 1), device="cuda"
        )
        self.offset_abs_gradient_accum = torch.zeros(
            (self.get_anchor.shape[0] * self.n_offsets, 1), device="cuda"
        )
        self.contrib_accum = torch.zeros((self.get_anchor.shape[0], 1), device="cuda")
        ...
        if self.use_feat_bank:
            l = [
                ...
                {
                    "params": self.mlp_color_time.parameters(),
                    "lr": training_args.mlp_color_lr_init,
                    "name": "mlp_color_time",
                },
                {
                ...
                },
                {
                    ...
                    "lr": training_args.uncertainty_lr,
                    "name": "mlp_uncertainty_model",
                },
            ]
        ...
        self.mlp_color_time_scheduler_args = get_expon_lr_func(
            lr_init=training_args.mlp_color_lr_init,
            lr_final=training_args.mlp_color_lr_final,
            lr_delay_mult=training_args.mlp_color_lr_delay_mult,
            max_steps=training_args.mlp_color_lr_max_steps,
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "mlp_color":
                ...
                param_group["lr"] = lr
            if param_group["name"] == "mlp_color_time":
                lr = self.mlp_color_time_scheduler_args(iteration)
                param_group["lr"] = lr
            ...
            if (
                ...
                and param_group["name"] == "embedding_appearance"
            ):
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate(
            (anchor, normals, offset, anchor_feat, opacities, scale, rotation), axis=1
        )
    ...
    def load_ply_sparse_gaussian(self, path):
        ...
        opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis].astype(
            np.float32
        )
    ...
    def training_statis(
        ...
        # print(opacity.shape, contrib.shape)

        temp_opacity = opacity.clone().view(-1).detach()
        temp_contrib = torch.zeros_like(temp_opacity)
        temp_opacity[temp_opacity < 0] = 0
        temp_contrib[temp_opacity > 0] = contrib[0, :].view(-1).detach()

        temp_opacity = temp_opacity.view([-1, self.n_offsets])
        self.opacity_accum[anchor_visible_mask] += temp_opacity.sum(dim=1, keepdim=True)

        temp_contrib = temp_contrib.view([-1, self.n_offsets])
        self.contrib_accum[anchor_visible_mask] += temp_contrib.sum(dim=1, keepdim=True)
        ...
        grad_norm_abs = torch.norm(
            viewspace_point_tensor.grad[update_filter, 2:], dim=-1, keepdim=True
        )
        self.offset_gradient_accum[combined_mask] += grad_norm
        self.offset_abs_gradient_accum[combined_mask] += grad_norm_abs
    ...
    def anchor_growing(self, grads, grads_div, threshold, offset_mask):
        ##
        ...
        for i in range(self.update_depth):
            ...
            candidate_mask = (grads >= cur_threshold).squeeze(-1)
            # candidate_ratio = torch.sum(grads >= cur_threshold) / grads.shape[0]

            # grads = grads / (torch.max(grads) + 1e-8)
            # grads_div = grads_div / (torch.max(grads_div) + 1e-8)

            # ratio = candidate_ratio if candidate_ratio < 2 else 2
            # Q_grad = torch.quantile(grads.reshape(-1), 1 - ratio / 2)
            # Q_grad_div = torch.quantile(grads_div.reshape(-1), 1 - ratio / 2)
            # candidate_mask = torch.where(grads >= Q_grad, True, False)
            # candidate_mask_div = torch.where(grads_div >= Q_grad_div, True, False)
            # candidate_mask = torch.logical_or(candidate_mask, candidate_mask_div).squeeze(-1)
            # print(torch.sum(candidate_mask))
            # print(candidate_mask.shape, offset_mask.shape)
            # candidate_mask = (grads >= cur_threshold)
            # grads = grads + grads_div
            # Q_grads = torch.quantile(grads_div.reshape(-1), 1 - candidate_ratio)
            # candidate_mask = (grads > Q_grads).squeeze(-1)
            # print(candidate_mask.shape, offset_mask.shape)
            ...
            if candidate_anchor.shape[0] > 0:
                ...
                new_rotation = torch.zeros(
                    [candidate_anchor.shape[0], 4], device=candidate_anchor.device
                ).float()
                new_rotation[:, 0] = 1.0
                ...
                new_opacities = inverse_sigmoid(
                    0.1
                    * torch.ones(
                        (candidate_anchor.shape[0], 1), dtype=torch.float, device="cuda"
                    )
                )
                ...
                del self.contrib_accum
                self.contrib_accum = temp_contrib_accum
    ...
    def adjust_anchor(
        ...
        grads_abs = self.offset_abs_gradient_accum / self.offset_denom
        grads_abs[grads_abs.isnan()] = 0.0
        # grads_abs = torch.norm(grads_abs, dim=-1)

        grads_div = grads_abs / (grads + 1e-7)
        grads_div[grads_div.isnan()] = 0.0
        # grads_div = torch.norm(grads_div, dim=-1)

        offset_mask = (
            self.offset_denom > check_interval * success_threshold * 0.5
        ).squeeze(dim=1)

        self.anchor_growing(grads, grads_div, grad_threshold, offset_mask)
        ...
        self.offset_abs_gradient_accum[offset_mask] = 0
        ...
        padding_offset_abs_gradient_accum = torch.zeros(
            [
                self.get_anchor.shape[0] * self.n_offsets
                - self.offset_gradient_accum.shape[0],
                1,
        ...
        self.offset_abs_gradient_accum = torch.cat(
            [self.offset_abs_gradient_accum, padding_offset_abs_gradient_accum], dim=0
        )

        # # prune anchors
        contrib_mask = (self.contrib_accum < 0.1 * self.anchor_demon).squeeze(dim=1)
        prune_mask = (self.opacity_accum < min_opacity * self.anchor_demon).squeeze(
            dim=1
        )
        ...
        offset_abs_gradient_accum = self.offset_abs_gradient_accum.view(
            [-1, self.n_offsets]
        )[~prune_mask]
        offset_abs_gradient_accum = offset_abs_gradient_accum.view([-1, 1])
        del self.offset_abs_gradient_accum
        self.offset_abs_gradient_accum = offset_abs_gradient_accum

        # update opacity accum
        if anchors_mask.sum() > 0:
            self.opacity_accum[anchors_mask] = torch.zeros(
                [anchors_mask.sum(), 1], device="cuda"
            ).float()
            self.contrib_accum[anchors_mask] = torch.zeros(
                [anchors_mask.sum(), 1], device="cuda"
            ).float()
        ...
        temp_contrib_demon = self.contrib_accum[~prune_mask]
        del self.contrib_accum
        self.contrib_accum = temp_contrib_demon
    ...
    def save_mlp_checkpoints(self, path, mode="split"):  # split or unite
        ...
        if mode == "split":
            ...
            self.mlp_color_time.eval()
            mlp_color_time = torch.jit.trace(
                self.mlp_color_time,
                (
                    torch.rand(
                        1,
            ...
            mlp_color_time.save(os.path.join(path, "color_mlp_time.pt"))
            self.mlp_color_time.train()

            if self.use_feat_bank:
                self.mlp_feature_bank.eval()
                feature_bank_mlp = torch.jit.trace(
                    self.mlp_feature_bank, (torch.rand(1, 3 + 1).cuda())
                )
                ...
                torch.save(
                    {
                        ...
                        "color_mlp_time": self.mlp_color_time.state_dict(),
        ...
        torch.save(
            self.uncertainty_model.state_dict(),
            os.path.join(path, "uncertainty_model.pth"),
        )
    ...
    def load_mlp_checkpoints(self, path, mode="split"):  # split or unite
        if mode == "split":
            ...
            self.mlp_color_time = torch.jit.load(
                os.path.join(path, "color_mlp_time.pt")
            ).cuda()
            if self.use_feat_bank:
            ...
            self.mlp_color_time.load_state_dict(checkpoint["color_mlp_time"])
            if self.use_feat_bank:
        ...
        ckpt = torch.load(os.path.join(path, "uncertainty_model.pth"))
        self.uncertainty_model.load_state_dict(ckpt)
