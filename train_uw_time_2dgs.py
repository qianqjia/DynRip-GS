...
from gaussian_renderer.render_time_2dgs import prefilter_voxel, render
import sys
...
from arguments.args_mine import ModelParams, PipelineParams, OptimizationParams
import cv2

def tensor2numpy(tensor, bgr2rgb=True):
    if bgr2rgb:
        return cv2.cvtColor(tensor.detach().permute(1, 2, 0).cpu().numpy(), cv2.COLOR_BGR2RGB)
...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if torch.isnan(image).any():
            print(iteration)
            exit()
        # img = tensor2numpy(image)
        # gt = tensor2numpy(gt_image)
        # cv2.imshow('image', img)
        # cv2.imshow('gt_image', gt)
        # cv2.waitKey(0)
        # save = np.concatenate([img, gt], axis=1)
        # os.makedirs('tmp', exist_ok=True)
        # cv2.imwrite('tmp/{}.png'.format(iteration), (save.clip(0, 1) * 255).astype(np.uint8))
        ...
        # torch.nn.utils.clip_grad_norm_(gaussians.get_parameters(), max_norm=1.0)
        loss.backward()
        ...
        with torch.no_grad():
            ...
            # Optimizer step
            if iteration < opt.iterations:
                torch.nn.utils.clip_grad_norm_(gaussians.get_mlp_parameters(), max_norm=1.0)
                gaussians.optimizer.step()
                gaussians.optimizer.zero_grad(set_to_none = True)
                def clamp_parameters(model, min_val=-1.0, max_val=1.0):
