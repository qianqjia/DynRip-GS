	...
	const bool debug)
{
  ...
  torch::Tensor contrib = torch::full({2, P}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		contrib.contiguous().data<float>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_others, radii, geomBuffer, binningBuffer, imgBuffer, contrib);
}
