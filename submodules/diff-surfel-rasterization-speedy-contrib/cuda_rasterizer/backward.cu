 ...
 */
	 ...
	 for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	 {
		 ...
		 for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		 {
			 ...
			 if (rho3d <= rho2d) {
				 ...
				 atomicAdd(&dL_dmean2D[global_id].z, abs(dL_dTu.z) + abs(dL_dTv.z));
			 } else {
				 ...
				 atomicAdd(&dL_dmean2D[global_id].z, abs(dL_dG * dG_ddelx) + abs(dL_dG * dG_ddely));
	 ...
	 glm::vec4* dL_drots)
 {
	 ...
	 dL_dmean2Ds[idx].z = dL_dmean2Ds[idx].z * depth; 
 }
